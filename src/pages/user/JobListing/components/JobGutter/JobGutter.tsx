...
function JobGutter() {
    const jobsApi = 'https://job-portal-server-e9q1.onrender.com/job?skillsRequire[in]=[%22NodeJS%22,%22ReactJS%22%20]'
    const [jobs, setJobs] = useState<any[]>([])

    useEffect(() => {
        axiosClient.get(jobsApi)
                        ...
                        <div className=' border-content-border border rounded p-4 relative'>
                            ...
                            <div className=' mt-5 mb-5 flex items-center'>
                                ...
                                <div className='flex flex-col'>
                                    <h3 className=' text-content-title font-semibold text-lg xl:text-base mb:text-cb'>{job.title}</h3>
                                    <div className=' text-content-text text-sm font-medium flex items-center xl:flex-col xl:items-start tb:flex-row mb:flex-col mb:items-start mb:text-xs '>
                                        ...
                                        <p className=' text-content-title font-semibold flex ml-5 mb:text-xs mb:ml-0 '>Deadline <span className=' font-medium'>: 05 April, 2023</span> </p>
                                    </div>
                                </div>
                            </div>
                            <div className=' list-none border-t border-content-border pt-5'>
                                <div className=' flex items-center mb-2 before:w-2 before:h-2 before:rounded-full before:bg-slate-300 before:mr-2'>
                                    <p className=' text-content-text font-medium text-cb lg:text-sm mb:text-xs '>Salary: <span className=' text-content-title font-semibold ml-1'>{job.salary.toLocaleString('it')}đ /</span> tháng</p>
                                </div>
                                <div className=' flex items-center mb-2 before:w-2 before:h-2 before:rounded-full before:bg-slate-300 before:mr-2'>
                                    <p className=' text-content-text font-medium text-cb lg:text-sm mb:text-xs '>Experience: <span className=' text-content-title font-semibold ml-1'>{job.skillsRequire}</span></p>
                                </div>
                                <div className=' flex items-center mb-2 before:w-2 before:h-2 before:rounded-full before:bg-slate-300 before:mr-2'>
                                    <p className=' text-content-text font-medium text-cb lg:text-sm mb:text-xs '>Location: <span className=' text-content-title font-semibold ml-1'>{job.postedBy.location}</span></p>
                                </div>
                            </div>
                            <div className=' mt-5 mb-3 flex relative'>
                                ...
                                <div className=' text-content-title text-sm font-semibold bg-content-bg-remote rounded-3xl pt-1.5 pb-1.5 pl-6 pr-6 lg:text-xs '>Remote</div>

                                <div className=' group bottom-2 right-0 absolute '>
                                    <Link to={'/job-detail/' + job.id} className=' text-primary-100 flex items-center relative group-hover:cursor-pointer '>
