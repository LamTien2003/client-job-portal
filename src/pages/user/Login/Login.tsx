...
const LOGIN_URL = 'https://job-portal-server-e9q1.onrender.com/auth/login'
...
const Login = () => {
    ...
    const handleSubmit = async (e:any) => {
        ...
        try {
            const reponse = await axiosClient.post(LOGIN_URL,
                {email, password}    
            )
            console.log(JSON.stringify(reponse?.data?.data))

            const accessToken = reponse?.data?.accessToken
            const roles = reponse?.data?.data?.role
